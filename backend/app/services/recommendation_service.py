...
class RecommendationService:
    ...
    def recommend_centres(
        ...
        centres = centre_repository.list_nearby(
            farmer_lat, farmer_lon, max_km=settings.MAX_REASONABLE_DISTANCE_KM
        )
        ...
        # (centre, distance, prediction, congestion_index, level, score_breakdown)
        scored: List[tuple] = []
        ...
        for centre in centres:
            ...
            prediction = self.wait_time_predictor.predict(
                centre.centre_id, farmer_lat, farmer_lon, at_time
            )
            ...
            scored.append((centre, distance_km, prediction, congestion_index, level, breakdown))

        # Rank by score descending (higher = better)
        scored.sort(key=lambda x: x[5].score, reverse=True)

        # Identify nearest-by-distance centre to contrast against
        nearest_entry = min(scored, key=lambda x: x[1])

        results: List[CentreRecommendation] = []
        for rank, (centre, distance_km, prediction, congestion_index, level, breakdown) in enumerate(scored, start=1):
            is_top = rank == 1
            comparison_name = None
            comparison_wait = None

            # Pull cached prediction from nearest entry instead of recomputing
            if is_top and nearest_entry[0].centre_id != centre.centre_id:
    ...
    # ------------------------------------------------------------------
    def best_time(self, centre_id: str, at_time: Optional[datetime] = None) -> BestTimeResponse:
        centre = self._get_centre_or_raise(centre_id)
        at_time = at_time or datetime.now(timezone.utc)

        slots = recommend_best_times(centre, at_time, settings.FORECAST_HORIZON_HOURS)
    ...
    # ------------------------------------------------------------------
    def get_congestion(self, centre_id: str) -> CongestionResponse:
        centre = self._get_centre_or_raise(centre_id)
    ...
    # ------------------------------------------------------------------
    def get_forecast(self, centre_id: str, at_time: Optional[datetime] = None) -> ForecastResponse:
        centre = self._get_centre_or_raise(centre_id)
        at_time = at_time or datetime.now(timezone.utc)

        points = forecast_next_hours(centre, at_time, settings.FORECAST_HORIZON_HOURS)
    ...
    # 6: Dynamic rerouting
    # ------------------------------------------------------------------
    def evaluate_reroute_for_token(self, token_id: str, at_time: Optional[datetime] = None) -> RerouteResponse:
        at_time = at_time or datetime.now(timezone.utc)

        token_info = self.queue_provider.get_token_info(token_id)
        current_centre = self._get_centre_or_raise(token_info.centre_id)

        current_prediction = self.wait_time_predictor.predict(
            current_centre.centre_id, token_info.farmer_lat, token_info.farmer_lon, at_time
        )

        nearby = centre_repository.list_nearby(
            token_info.farmer_lat, token_info.farmer_lon, max_km=settings.MAX_REASONABLE_DISTANCE_KM
        )
        alternatives = [
            c for c in nearby
        ...
        if alternatives:
            best_alt = min(
                (
                    (alt, self.wait_time_predictor.predict(
                        alt.centre_id, token_info.farmer_lat, token_info.farmer_lon, at_time
                    ).eta_minutes)
                    for alt in alternatives
                ),
                key=lambda item: item[1],
            )
            best_alt_centre_id, best_alt_eta = best_alt[0].centre_id, best_alt[1]
    ...
    # ------------------------------------------------------------------
    def _get_centre_or_raise(self, centre_id: str) -> CentreSnapshot:
